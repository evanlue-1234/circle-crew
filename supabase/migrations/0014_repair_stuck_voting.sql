-- advance_plan_to_voting previously refused to touch a plan that was already 'voting',
-- treating that purely as "already done, nothing to do." That's right when voting_ideas is
-- actually populated, but a plan can end up at status='voting' with voting_ideas still null
-- (e.g. from testing against an earlier version of this function) with no way to recover,
-- since nothing else is allowed to write these columns anymore (migration 0013's REVOKE).
-- This makes the function self-healing: a 'voting' plan with no voting_ideas gets its
-- candidates/best_day/best_time (re)computed instead of being permanently stuck.
